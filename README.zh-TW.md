...
- 你叫 AI Agent 讀一篇公開的 Facebook 貼文——它回你*「抱歉，我無法擷取這個網頁」*。
- 工作流每次卡在這裡。你只好自己開瀏覽器、複製貼上、再餵回去。
- 內容明明是公開的，Agent 就是不知道怎麼繞過各平台的防護。
- 每個平台的門鎖不一樣，一把鑰匙打不開所有的門。

## 解法

一支 Python 腳本，內建**四層備援策略**，自動處理各平台的防護機制。
...
- **裸抓優先** ——能不開瀏覽器就不開，省資源省時間。
- **策略分離** ——每個網站有自己的抓取策略，新增網站只要加一筆設定。
- **失敗就說** ——不會假裝成功，抓不到會明確告訴你為什麼、建議怎麼做。
- **只碰公開的** ——不登入、不存憑證、不繞過任何防護。
